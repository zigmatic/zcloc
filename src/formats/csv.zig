...
//! With `--percent`, adds: pct_code, pct_comment, pct_blank, pct_of_total.
...
pub fn write(writer: anytype, summary: *const stats.Summary) !void {
    if (summary.percent) {
        try writer.writeAll("language,files,blank,comment,code,pct_code,pct_comment,pct_blank,pct_of_total\n");
    } else {
    ...
    for (summary.by_language) |lang| {
        if (summary.percent) {
            ...
            try writer.print("{s},{d},{d},{d},{d},{d:.1},{d:.1},{d:.1},{d:.1}\n", .{
...
test "write produces CSV with percentages" {
    var buf: [4096]u8 = undefined;
    var writer = std.Io.Writer.fixed(&buf);

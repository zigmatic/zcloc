...
pub fn count(content: []const u8, lang: *const reg.LanguageDefinition) Counts {
    ...
    while (line_it.next()) |raw_line| {
        ...
        if (in_block_comment) {
            const ended_with_code = processBlockContent(line, lang, &in_block_comment, &block_depth);
            if (ended_with_code) {
                counts.code += 1;
            } else {
                counts.comment += 1;
        ...
        if (lang.block_comment_start.len > 0 and startsWith(trimmed, lang.block_comment_start)) {
            ...
            const ended_with_code = processBlockContent(after_start, lang, &in_block_comment, &block_depth);
            if (ended_with_code) {
                counts.code += 1;
            } else {
                counts.comment += 1;
...
/// Processes a line while inside a block comment. Returns true if the block
/// comment ended on this line and the remaining content is code.
fn processBlockContent(line: []const u8, lang: *const reg.LanguageDefinition, in_block: *bool, depth: *u32) bool {
    var pos: usize = 0;
